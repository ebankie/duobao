$(function () {
    ...
    socket.on('new_msg' , function (msg) {  // 后端推送来消息时
        ...
        if (msg.code == 1) {   //出价成功
            $('span[data-goodsId=' + msg.data.goodsId + ']').text('￥' + msg.data.nowPrice);
        }
    });

    var sv = $('select[name=select_type] option:selected').val();
    if(sv==1){
        $('select[name=range]').show();
        $('select[name=cate]').hide();
    }else{
        $('select[name=range]').hide();
        $('select[name=cate]').show();
    }
    $('select[name=select_type]').change(function () {
        var value = $(this).val();
        $('input[name=select_type]').val(value);
        if(value==1){
            $('select[name=range]').show();
            $('select[name=cate]').hide();
        }else{
...
function updateEndTime() {
    var date = new Date();
    var time = date.getTime();  //当前时间距1970年1月1日之间的毫秒数；客户端当前时间戳
    ...
    $(".settime").each(function (i) {
        ...
        if (time <= startTime) {
            ...
            if (lag > 0) {
                ...
                $(this).html("<t style='color:#eab881;'>已结束</t>");
                $(this).next('p').children('span').css('color' , '#ADADAD');
            }
        }
