...
function send($url , $post_data = array ()){
    $ch = curl_init();
...
function session($name = '' , $value = ''){
    ...
    if (is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if (C('SESSION_TYPE')) { // 读取session驱动
            ...
            $hander = new $class();
            session_set_save_handler(array (&$hander , "open") , array (&$hander , "close") , array (&$hander , "read") , array (
                &$hander ,
                "write"
            ) , array (&$hander , "destroy") , array (&$hander , "gc"));
        }
        // 启动session
...
function cookie($name = '' , $value = '' , $option = NULL){
    ...
    if (!is_null($option)) {
        if (is_numeric($option))
            $option = array ('expire' => $option); elseif (is_string($option))
